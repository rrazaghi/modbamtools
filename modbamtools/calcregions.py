...
# class MethRegion:
#     """
#     The clusterer class runs clustering algorithm on input modifed bases regions

#     """
# def __init__(self, bam, bed, , output, threads, min_calls=5, min_cov=80) -> None:
#     self.bam = bam
#     self.bed = bed
#     self.output = output
...
# def parse_bed_out_stats_horizontal(self):
#     data = []
...
#     s = p.starmap(get_region_stats_hap_horizontal, zip(repeat(self.bam), data))
#     results.extend(s)
#     p.close()
#     p.join()
...
# def get_region_stats_hap_horizontal(bam, bed_line):

#     chrom = bed_line[0]
#     start = int(bed_line[1])
#     end = int(bed_line[2])

#     mean,std,cov = get_region_stats_horizontal(bam, chrom, start, end)
#     hp1_mean,hp1_std,hp1_cov = get_region_stats_horizontal(bam, chrom, start, end, tag_name="HP", tag_value=1)
#     hp2_mean,hp2_std,hp2_cov = get_region_stats_horizontal(bam, chrom, start, end, tag_name="HP", tag_value=2)

#     bed_line.extend(map(str,[mean, std, cov, hp1_mean,hp1_std,hp1_cov, hp2_mean, hp2_std, hp2_cov]))
